...
import com.lumina.domain.book.entity.*;
import com.lumina.domain.book.repository.*;
...
public class BookService {
    ...
    public BookResponse create(UUID userId, CreateBookRequest request) {
        ...
        ReadingStatus status = StringUtils.hasText(request.status()) ? parseStatus(request.status()) : ReadingStatus.WANT_TO_READ;
    ...
    public BookResponse update(UUID userId, UUID bookId, UpdateBookRequest request) {
        ...
        if (request.author() != null) book.setAuthor(trimToNull(request.author()));
        if (request.coverUrl() != null) book.setCoverUrl(trimToNull(request.coverUrl()));
        if (request.totalPages() != null) {
            validatePages(request.totalPages());
            book.setTotalPages(request.totalPages());
        }
        if (request.currentPage() != null) setCurrentPage(book, request.currentPage());
        if (request.status() != null) applyStatus(book, parseStatus(request.status()));
        if (request.rating() != null) {
            if (request.rating() < 1 || request.rating() > 5) throw validation("A avaliação deve estar entre 1 e 5");
            book.setRating(request.rating());
        }
        if (request.review() != null) book.setReview(trimToNull(request.review()));
        if (request.genre() != null) book.setGenre(trimToNull(request.genre()));
        if (request.tags() != null) book.setTags(cleanTags(request.tags()));
    ...
    public BookResponse logReading(UUID userId, UUID bookId, ReadingLogRequest request) {
        ...
        if (book.getStatus() == ReadingStatus.WANT_TO_READ) applyStatus(book, ReadingStatus.READING);
    ...
    public List<BookSearchResultResponse> search(String query) {
        ...
        try {
            ...
            if (root == null || !root.path("items").isArray()) return List.of();
            ...
            root.path("items").forEach(item -> {
                ...
                String author = volume.path("authors").isArray() && !volume.path("authors").isEmpty()
                    ? volume.path("authors").get(0).asText() : null;
                String genre = volume.path("categories").isArray() && !volume.path("categories").isEmpty()
                    ? volume.path("categories").get(0).asText() : null;
    ...
    private void applyStatus(Book book, ReadingStatus status) {
        ...
        if (isReading(status) && book.getStartedAt() == null) book.setStartedAt(LocalDate.now());
        if (status == ReadingStatus.COMPLETED) {
            book.setFinishedAt(LocalDate.now());
            if (book.getTotalPages() != null) book.setCurrentPage(book.getTotalPages());
        } else {
    ...
    private void setCurrentPage(Book book, int page) {
        if (page < 0) throw validation("A página atual não pode ser negativa");
    ...
    private void validatePages(Integer pages) {
        if (pages != null && pages < 1) throw validation("O total de páginas deve ser maior que zero");
    }

    private ReadingStatus parseStatus(String value) {
        try { return ReadingStatus.valueOf(value.trim().toUpperCase(Locale.ROOT)); }
        catch (RuntimeException exception) { throw validation("Status de leitura inválido"); }
    }
